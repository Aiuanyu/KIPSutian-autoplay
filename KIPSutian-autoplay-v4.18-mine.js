...
// @namespace    aiuanyu
// @version      4.18（自己找問題）
// @description  自動開啟查詢結果表格中每個詞目連結於 Modal iframe，依序播放音檔(自動偵測時長)，主表格自動滾動高亮，**處理完畢後自動跳轉下一頁繼續播放(修正URL與啟動時機)**，可即時暫停/停止/點擊背景暫停/點擊表格列播放，並根據亮暗模式高亮按鈕。 **v4.18: 暫時簡化 TABLE_CONTAINER_SELECTOR 以排查容器移除問題後，把容器選擇器變為 3 種。**
...
(function () {
  ...
  // const TABLE_CONTAINER_SELECTOR = 'main.container-fluid div.mb-5'; // 似乎不能只用 1 種容器選擇器耶！
  const TABLE_CONTAINER_SELECTOR_1 = 'main.container-fluid div.mb-5'; // 此處把容器選擇器三分
  const TABLE_CONTAINER_SELECTOR_2 = 'main.container-fluid div.mt-1.mb-5';
  const TABLE_CONTAINER_SELECTOR_3 = 'main.container-fluid div.mt-1.mb-4';
  const ALL_TABLES_SELECTOR = `${TABLE_CONTAINER_SELECTOR_1} > table, ${TABLE_CONTAINER_SELECTOR_2} > table, ${TABLE_CONTAINER_SELECTOR_3} > table`; // 3 種容器選擇器分別加上 table，不然只有最後 1 種加 table 啊！
